node() {
	...
	stage("deploy the test container") {
		...
		def image = "${item.status.dockerImageRepository}:PrepareForTesting"
        println image
		sh "oc create dc ${microservice} --image=172.30.1.1:5000/test/project-api:PrepareForTesting -n ${project}"
